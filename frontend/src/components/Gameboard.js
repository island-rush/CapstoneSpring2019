...
function singleHex(id, pattern, x, y, selectPos, selectedPos, pieces, highlighted, highlightedType, plannedPos) {
    return (
        <Hexagon posId={id} q={x} r={y} s={-999} fill={patternSelector(pattern, pieces)} 
          // className={selectedPos === id || (highlighted.includes(id) && (highlightedType, this.props.plannedPos === "all" || highlightedType, this.props.plannedPos === pattern)) ? "selectedPos" : plannedPos.includes(id) ? "plannedPos" : ""} 
          className={plannedPos.includes(id) ? "plannedPos" : (selectedPos === id || (highlighted.includes(id) && (highlightedType, this.props.plannedPos === "all" || highlightedType, this.props.plannedPos === pattern))) ? "selectedPos" : ""} 
